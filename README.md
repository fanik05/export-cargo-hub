...
src/
  app/                        App Router routes, layouts, and global styles
    ...
    (admin)/admin/            Admin route group (session-gated)
      page.tsx                Shipment list with preview drawer
  ...
  lib/
    ...
    format.ts                 Date and label formatting (UTC, fixed month table)
  ...
  proxy.ts                    Redirects unauthenticated /admin/* to /login
prisma/
...
Imports use the `@/*` alias, which maps to `src/` — so `@/components/ui/button`,
`@/lib/prisma`.
...
3. **Mutations are Server Actions.** Creates and updates live in `src/actions/` and
   validate their input with Zod. Route Handlers (`src/app/api/`) are reserved for
   file downloads (PDFs) and public tracking endpoints.
4. **Reuse the UI kit.** Check `src/components/ui/` for an existing shadcn primitive
